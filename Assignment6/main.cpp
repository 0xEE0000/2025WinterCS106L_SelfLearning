...
public:
  ...
  std::optional<Course> find_course(std::string course_title)
  {
     ...
     * type. */
    if(course_title.empty()) return std::nullopt;
...
main(int argc, char* argv[])
{
  static_assert(
    !std::is_same_v<std::invoke_result_t<decltype (&CourseDatabase::find_course), 
                    ...
                    std::optional<std::vector<Course>>>,
  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = course
                         .transform([](Course& course)->std::string{return "Found course: "+course.title+","+course.number_of_units+","+course.quarter;})
