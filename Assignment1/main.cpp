  ...
  /* (STUDENT TODO) Your code goes here... */
	...
	if(ifs.is_open()){
		...
		while(getline(ifs,line)){
			...
			course_info=split(line,',');
			std::string quarter=course_info.back();
			course_info.pop_back();
			std::string number_of_units=course_info.back();
			course_info.pop_back();
			std::string title=course_info.back();
	...
	if(ofs.is_open()){
		...
		for(int i=0;i<all_courses.size();i++){
			if(all_courses[i].quarter!="null"){
				ofs <<all_courses[i].title<<','<<all_courses[i].number_of_units<<','<<all_courses[i].quarter<<'\n';
 ...
 * `unlisted_courses` wiil trivially contain classes that are not offered
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
	...
	if(ofs.is_open()){
		ofs<<"Title,Number of Units,Quarter\n";
		for(const auto& a1:unlisted_courses){
			ofs<<a1.title<<','<<a1.number_of_units<<','<<a1.quarter<<'\n';
		}
	}
	ofs.close();
}
